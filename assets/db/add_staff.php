<?php
    ...
    $INSERT = "INSERT INTO staffs (staff_id, name, username, password) 
           VALUES (?, ?, ?, ?) 
    ...
    if ($rnum == 0) {
        ...
        $stmt->bind_param("ssss", $staff_id, $staff_name, $staff_username, $secure_pass);
        $stmt->execute();

        $conn->query('INSERT INTO users 
        (user_type, username, password, status) 
        VALUES 
        ("'.$staff_type.'", "'.$staff_username.'", "'.$secure_pass.'", "active")');
        $response = [
            'status' => 'success',
