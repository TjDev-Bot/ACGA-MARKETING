<?php
    ...
    switch ($type) {
        case 'hold':
            ...
            $query = $conn->query("SELECT held_orders.product_id, held_orders.quantity, branch_inventory.unit_price FROM held_orders 
            ...
            while($row = $query->fetch_assoc()){
                $price = intval($row['quantity']) * intval($row['unit_price']);
                // $total += $totalprice;
        ...
        case 'purchase':
                ...
                $date = date("Y-m-d");
                $query = $conn->query("SELECT checkout.product_id, checkout.quantity, branch_inventory.unit_price FROM checkout INNER JOIN branch_inventory ON checkout.product_id = branch_inventory.id WHERE checkout.id = $product_id");
                while($row = $query->fetch_assoc()){
